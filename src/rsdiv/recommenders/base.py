...
class BaseRecommender(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self.df_interaction, self.user_array, self.item_array = self.get_interaction(df_interaction)
    ...
    def get_interaction(self, df_interaction: pd.DataFrame) -> Tuple[pd.DataFrame, np.ndarray, np.ndarray]:
    ...
    def process_interaction(self) -> Tuple[sps.coo_matrix, Optional[sps.coo_matrix]]:
        ...
        if self.test_size != 0:
            train, test = train_test_split(dataframe, test_size=self.test_size, shuffle=False)
    ...
    def clean_items(self) -> pd.DataFrame:
        ...
        self.items["encodes"] = self.items["itemId"].apply(lambda x: self._encode(x, invmap))
    ...
    @abstractmethod
    def predict(self, user_ids: np.ndarray, item_ids: np.ndarray, user_features: Optional[sps.csr_matrix], item_features: Optional[sps.csr_matrix]) -> np.ndarray:
        raise NotImplementedError("predict must be implemented.")

    @staticmethod
    def _normalize_scores(scores: np.ndarray, feature_range: Tuple[int, int] = (0, 1)) -> np.ndarray:
        scores = minmax_scale(scores, feature_range=feature_range, axis=0, copy=True)
        return scores

    def _get_toppop(self, toppop_keep: Optional[np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def predict_for_userId(self, user_id: int) -> np.ndarray:
        ...
        prediction = self.predict(user_ids, item_ids, self.user_features, self.item_features)
        return prediction

    def predict_for_userId_unseen(self, user_id: int) -> np.ndarray:
        seen = set(self.df_interaction[self.df_interaction["userId"] == user_id]["itemId"])
    ...
    def predict_top_n_item(self, user_id: int, top_n: int) -> pd.DataFrame:
        ...
        items = items.loc[items.encodes.isin(candidates.itemId)].drop(columns=["encodes"])
        return pd.merge(candidates, items, on="itemId").sort_values(by=["prediction"], ascending=False)
